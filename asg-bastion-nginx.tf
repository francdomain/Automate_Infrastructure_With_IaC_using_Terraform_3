...
resource "aws_sns_topic" "fnc-sns" {
  name = "Default_CloudWatch_Alarms_Topic"
}

# creating notification for all the auto scaling groups
resource "aws_autoscaling_notification" "fnc_notifications" {
  group_names = [
  ...
  topic_arn = aws_sns_topic.fnc-sns.arn
}
...
resource "aws_launch_template" "bastion-launch-template" {
  name                   = "${var.name}-bastion-template"
  image_id               = var.ami
